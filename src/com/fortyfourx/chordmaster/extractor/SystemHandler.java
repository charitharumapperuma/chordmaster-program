...
/*
 * String url; while (true) { if(SystemHandler.urlPool.hasNext()) { synchronized
 * (SystemHandler.urlPool) { url = SystemHandler.urlPool.next(); }
 * 
 * scraper = new Scraper(url); executor.execute(scraper); } else { if
 * (SystemHandler.browserPool.isIdle()) { break; } } }
 */

// OLD ERROR HANDLE CODE
/*
 * //TODO: change as in handle() and main() methods. No separate browser pools.
 * Instantiating moved to main method. public void handleExceptions() {
 * System.out.println("started exception url handling...");
 * 
 * browserPool = new BrowserPool(SystemHandler.BROWSER_POOL_SIZE); urlPool = new
 * UrlPool(); synchronized (SystemHandler.urlPool) { urlPool.addAll((new
 * DatabaseHandler()).getAllErrorUrls()); }
 * 
 * Runnable runner;
 * 
 * String url; while (true) { if(SystemHandler.urlPool.hasNext()) { synchronized
 * (SystemHandler.urlPool) { url = SystemHandler.urlPool.next(); }
 * 
 * runner = new Scraper(url); executor.execute(runner); } else { if
 * (SystemHandler.browserPool.isIdle()) { break; } } }
 * 
 * browserPool.closeAll(); executor.shutdown();
 * 
 * System.out.println("completed exception url handling..."); }
 */

// IN OLD MAIN METHOD
/*
 * //h.executor = Executors.newFixedThreadPool(SystemHandler.BROWSER_POOL_SIZE);
 */
...
List<URL> urls = dbhandler.getAllUrls();
...
for (URL url : urls) {
	status = 0;
	id = Integer.parseInt(url.getUrl().split("=")[1]);

	song = dbhandler.getSongById(id);
	details = new File(SystemHandler.DETAILS_FOLDER_PATH + id + ".png");
	lyrics = new File(SystemHandler.LYRICS_FOLDER_PATH + id + ".png");

	if ((song != null) && details.exists() && lyrics.exists()) {
		status = 3;
	} else if ((song != null) && details.exists()) {
		status = 2;
	} else if (song != null) {
		status = 1;
	} else if (song == null) {
	...
	if (url.getStatus() != status) {
		dbhandler
				.addError(
						url.getUrl(),
						"com.fortyfourx.chordmaster.extractor.SystemHandler.validate() - Not all files are present.\nstatus = "
								+ status);
		dbhandler.updateUrl(url.getUrl(), status);
	}
}

System.out.println("Validator completed...");
*/